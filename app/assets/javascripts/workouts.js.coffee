...
Workouts.remove_fields = (link) ->
  ...
  $(link).parents("tr.fields").find(".set-field").removeClass("set-field") # remove the way in which we find the sets (since theyre simply hidden not removed)
...
Workouts.add_fields = (link, association, content) ->
  ...
  $(link).parents(".prev-placeholder").before(content.replace(regexp, new_id))
